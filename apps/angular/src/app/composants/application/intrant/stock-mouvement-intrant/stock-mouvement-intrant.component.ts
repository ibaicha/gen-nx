...
export class StockMouvementIntrantComponent {
  ...
  */
  ...
  chargeExploitationEnCours: string | null = null
  chargeExploitationUniteEnCours: string | null = null
  ...
  constructor(
    ...
    this.selectedPointId = this.appService.getLocalselectedCampagneAnneeId()
  ...
  prepareSelectedChargeExploitations() {
    ...
    const typeChargeExploitationSet: Set<string> = new Set(
      chargeExploitationsFiltres
        .map((paquet) => paquet.typeChargeExploitation?.name)
  ...
  preparePointsEntrepotEmplacements() {
    this.points.forEach((point) => {
      ...
      const mesEntrepots = point.Entrepot?.filter((entrepot) => {
        return entrepot.pointId === point.id
      })

      point.Entrepot?.forEach((entrepot) => {
        pointsGroupEntrepotEmplacements.push({
          entrepotId: entrepot.id,
          entrepotName: entrepot.name,
          emplacements: entrepot.Emplacement, //.emplacements,
        })
      })
    ...
    this.mouvementIntrantWithFiltersSubscription =
      this.mouvementIntrantWithFilters$.subscribe((datas: any[]) => {
        ...
        this.filteredPoints.forEach((item) => {
          ...
          item.Entrepot?.forEach((entrepot) => {
