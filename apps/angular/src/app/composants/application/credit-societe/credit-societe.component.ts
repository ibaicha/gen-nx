...
import { GetCreditParamsDTO } from '../../../store/credit/credit.model'
...
import { IAgence, IAnnee, IExploitation, IOp, IPointAgence, ISaison, ISociete, IVariete } from '@shared-models'
...
export class CreditSocieteComponent {
  ...
  filterParams: GetCreditParamsDTO = {
    societeId: undefined,
    agenceId: undefined,
    anneeId: undefined,
    saisonId: undefined,
  ...
  private initDispatch(): void {
    ...
    this.filterParams.anneeId = this.appService.getLocalselectedAnneeId()
      ? this.appService.getLocalselectedAnneeId().split(',').map(Number)
      : undefined
    this.filterParams.opId = this.appService.getLocalselectedOpId()
      ? this.appService.getLocalselectedOpId().split(',').map(Number)
      : undefined
    this.filterParams.societeId = this.appService.getLocalselectedSocieteId()
      ? this.appService.getLocalselectedSocieteId().split(',').map(Number)
      : undefined
    this.filterParams.agenceId = this.appService.getLocalselectedAgenceId()
      ? this.appService.getLocalselectedAgenceId().split(',').map(Number)
      : undefined
    this.filterParams.saisonId = this.appService.getLocalselectedSaisonId()
      ? this.appService.getLocalselectedSaisonId().split(',').map(Number)
      : undefined

    console.log('filterParams before: ', this.filterParams)
    this.appService.removeNullProperties(this.filterParams)
    console.log('filterParams after: ', this.filterParams)
  ...
  private initSubscriptions(): void {
    ...
    */
    ...
    this.creditWithFilters$ = this.store.pipe(
      select((state) => fromCredits.selectFilteredCreditWithParams(state, {})),
    )
            ...
            this.creditWithFilters$.subscribe((datas: any[]) => {
              if (datas) {
                ...
                */
                this.pointAgences.forEach((item) => {
                  const filterCreditsOnClientOnSociete: ICreditCustom[] =
                    this.creditWithFilters.filter(
                      (myCreditCustom) =>
                        myCreditCustom.exploitationOpPointId === item.pointId,
                    )
                  ...
                  const creditSumPointRemboursements =
                    filterCreditsOnClientOnSociete.reduce(
                  ...
                  this.creditSumGroupsPoint[item.pointId] = mySumCreditPoint
                })

                const creditSumSocieteCapitals = this.creditWithFilters.reduce(
                  (sum, credit) => sum + credit.capital,
                  0,
                )
                ...
                const creditSumSocieteInterets = this.creditWithFilters.reduce(
                  (sum, credit) => sum + credit.interet,
                  0,
                )
                ...
                this.agences.forEach((item) => {
                  ...
                  const creditSumAgenceRemboursements =
                    filterCreditsOnClientOnAgenceFinancier.reduce(
                      (sum, credit) => sum + credit.remboursementsSum,
                      0,
                    )
  ...
  filterOp(event: AutoCompleteCompleteEvent): void {
    const filtered: any[] = []
    const query = event.query
    const colonnesOps = this.ops.map(
      ({ id, name }): { id: number; name: string } => ({ id, name }),
    )

    for (const op of colonnesOps) {
      if (op.name.toLowerCase().includes(query.toLowerCase())) {
        filtered.push(op)
      }
    }
