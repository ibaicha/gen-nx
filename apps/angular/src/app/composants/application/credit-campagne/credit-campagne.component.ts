...
export class CreditCampagneComponent {
  ...
  selectedmesMouvementStockagesProduitCampagne: IMouvementStockage[] | undefined
  ...
  displayMouvementStockages = false
  titleDialogRMouvementStockages = ''
    ...
    this.creditsOnClient$ = this.store.pipe(
      select(
        fromCredits.selectCreditsCustomListFromAgence(
          ...
          this.loginService.mySelectedAnnee!.id,
          this.loginService.mySelectedSaison!.id,
  ...
  private initSubscriptions(): void {
    ...
    const groupAucun: SelectItem = {
      label: 'AUCUN',
      value: undefined,
      items: [{ label: '-- Select Variéte --', value: undefined }],
    }
    ...
    this.creditWithFilters$ = this.store.pipe(
      select(fromCredits.selectFilteredCreditWithParams, {}),
    )
    ...
    this.creditWithFiltersSubscription = this.creditWithFilters$.subscribe(
      (datas: any[]) => {
        if (datas) {
              ...
              next: (datas: any[]) => {
                if (datas) {
                  ...
                  datas.forEach((item) => {
                    ...
                    const mySumCapitals = creditsOnPoint.reduce(
                      (sum, credit) => sum + credit.capital,
                      0,
                    )

                    const mySumMoratoires = creditsOnPoint.reduce(
                      (sum, credit) => sum + credit.moratoire,
                      0,
                    )

                    const mySumInterets = creditsOnPoint.reduce(
                      (sum, credit) => sum + credit.interet,
                      0,
                    )

                    const mySumAutresEngagements = creditsOnPoint.reduce(
                    ...
                    const mySumRemboursements = creditsOnPoint.reduce(
          ...
          console.log('this.creditWithFilters datas:  ', this.creditWithFilters)
        }
      },
    )
  ...
  onSelectItemGroupVariete(event: any) {
    ...
    if (event.value == null || event.value == undefined) {
      ...
      this.selectedVarieteId = undefined
      this.loginService.mySelectedVariete = undefined
    } else {
  ...
  filterOp(event: AutoCompleteCompleteEvent) {
    ...
    const colonnesOps = this.mesOps.map(({ id, name }) => ({ id, name }))
  ...
  supprimerCredit() {
    ...
    body.anneeId = this.loginService.mySelectedAnnee!.id
    body.saisonId = this.loginService.mySelectedAnnee!.id
    body.producteurId = 0
