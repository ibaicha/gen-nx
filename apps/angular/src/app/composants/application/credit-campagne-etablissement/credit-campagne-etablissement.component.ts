...
export class CreditCampagneEtablissementComponent {
  ...
  PageNames = PageNames
  dialogPageIndex: PageNames = PageNames.DebutPage
  ...
  annees: IAnnee[] = []
  selectedAnnee?: IAnnee

  varietes: IVariete[] = []
  selectedVarieteId?: number
  produitDict: { [key: string]: IVariete[] } = {}

  selectItemGroupVarietes: SelectItem[] = []
  selectedVariete?: IVariete

  campagnes$!: Observable<ICampagne[]>
  campagnes: ICampagne[] = []
  selectedCampagne?: ICampagne

  saisons: ISaison[] = []
  selectedSaison?: ISaison

  societes$!: Observable<ISociete[]>
  societes: ISociete[] = []
  societe: any = {}
  selectedSociete?: ISociete

  agences: IAgence[] = []

  exploitations$!: Observable<IExploitation[]>
  private exploitationsSubscription?: Subscription
  exploitations: IExploitation[] = []
  exploitation: any = {}
  selectedExploitations?: IExploitation

  exploitationsCredits$!: Observable<IExploitationCredit[]>
  private exploitationsCreditsSubscription?: Subscription
  exploitationsCredits: IExploitationCredit[] = []
  exploitationCredit: any = {}
  selectedExploitationsCredits?: IExploitationCredit

  creditWithFilters$!: Observable<ICreditCustom[]>
  private creditWithFiltersSubscription?: Subscription
  creditWithFilters: ICreditCustom[] = []
  creditWithFilter: any = {}
  selectedcreditWithFilters?: ICreditCustom[]

  creditsOnClient$!: Observable<ICreditCustom[]>
  private creditsOnClientSubscription?: Subscription
  creditsOnClient: ICreditCustom[] = []
  creditOnClient: any = {}
  selectedCreditsOnClient?: ICreditCustom[]

  creditsOnPoint$!: Observable<ICreditCustom[]>
  private creditsOnPointSubscription?: Subscription
  creditsOnPoint: ICreditCustom[] = []

  creditsOnClientOnSociete$!: Observable<ICreditCustom[]>
  private creditsOnClientOnSocieteSubscription?: Subscription
  ...
  private pointAgenceSubscription?: Subscription
  pointAgence: IPointAgence[] = []
  pointFromAgence: any = {}
  selectedpointAgence?: IPointAgence[]
  ...
  private remboursementsSubscription?: Subscription
  remboursements: IRemboursement[] = []
  remboursement: any = {}
  selectedremboursements?: IRemboursement[]

  mesRemboursements$!: Observable<IRemboursement[]>
  private mesRemboursementsSubscription?: Subscription
  mesRemboursements: IRemboursement[] = []
  monRemboursement: any = {}
  selectedmesRemboursements?: IRemboursement[]
  ...
  private mouvementStockagesProduitCampagneSubscription?: Subscription
  mouvementStockagesProduitCampagne: IMouvementStockage[] = []
  mouvementStockageProduitCampagne: any = {}
  selectedmouvementStockagesProduitCampagne?: IMouvementStockage[]

  mesMouvementStockagesProduitCampagne$!: Observable<IMouvementStockage[]>
  private mesMouvementStockagesProduitCampagneSubscription?: Subscription
  mesMouvementStockagesProduitCampagne: IMouvementStockage[] = []
  monMouvementStockageProduitCampagne: any = {}
  selectedmesMouvementStockagesProduitCampagne?: IMouvementStockage[]
  ...
  display = false
  displayRemboursements = false
  titleDialogCredit = ''
  titleDialogRemboursements = ''
  iconeSave = 'pi pi-save'

  displayExploitation = false
  ...
  newCompte = ''
  titreCompte = "COMPTE D'EXPLOITATION"
  isEditedCredit = false
  isCompteCreated = false
  ...
  constructor(
    ...
    public clientService: OpService,
    ...
    this.creditsOnClient$ = this.store.pipe(
      select(
        fromCredits.selectCreditsCustomListFromAgence(
          ...
          this.loginService.selectedCampagne.anneeId,
          this.loginService.selectedCampagne.saisonId,
        ),
      ),
    )
  ...
  refreshPage() {
    ...
    this.mouvementStockagesProduitCampagne$ = this.store.pipe(
      select(
        fromMouvementStockages.selectMouvementStockagesProduitCampagneList,
      ),
    )
    ...
    this.pointAgenceSubscription = this.pointAgence$.subscribe({
      next: (datas: any[]) => {
        if (datas) {
          ...
          const mappedObj = sourceObj.pipe(
            map((x) => {
              return {
                ...
                adresse: x.agence.adresse as string,
                email: x.agence.email as string,
                telephone: x.agence.telephone as string,
                latitude: x.agence.latitude as number,
                longitude: x.agence.longitude as number,
                prenomContact: x.agence.prenomContact as string,
                nomContact: x.agence.nomContact as string,
                emailContact: x.agence.emailContact as string,
                telephoneContact: x.agence.telephoneContact as string,
                societeId: x.agence.societeId as number,
          ...
          this.creditsOnClientOnSocieteSubscription =
            this.creditsOnClientOnSociete$.subscribe((datas: any[]) => {
              if (datas) {
                this.creditsOnClientOnSociete = datas.map((device: any) => {
                  return { ...device }
                })
                ...
                this.pointAgence.forEach((item) => {
                  const filterCreditsOnClientOnSociete: ICreditCustom[] =
                    this.creditsOnClientOnSociete.filter(
                      (point) => point.exploitationOpPointId === item.pointId,
                    )

                  this.creditGroups[item.pointId] =
                    filterCreditsOnClientOnSociete

                  const creditSumPointCapitals =
                    filterCreditsOnClientOnSociete.reduce(
                      (sum, credit) => sum + credit.capital,
                      0,
                    )

                  const creditSumPointMoratoires =
                    filterCreditsOnClientOnSociete.reduce(
                      (sum, credit) => sum + credit.moratoire,
                      0,
                    )
                  const creditSumPointInterets =
                    filterCreditsOnClientOnSociete.reduce(
                      (sum, credit) => sum + credit.interet,
                      0,
                    )
                  const creditSumPointAutresEngagements =
                    filterCreditsOnClientOnSociete.reduce(
                      (sum, credit) => sum + credit.autres_engagements,
                      0,
                    )
                  const creditSumPointExigibles =
                  ...
                  this.creditSumGroupsPointCollect[item.pointId] =
                    mySumCreditPoint
                })

                const creditSumSocieteCapitals =
                  this.creditsOnClientOnSociete.reduce(
                    (sum, credit) => sum + credit.capital,
                    0,
                  )

                const creditSumSocieteMoratoires =
                  this.creditsOnClientOnSociete.reduce(
                    (sum, credit) => sum + credit.moratoire,
                    0,
                  )
                const creditSumSocieteInterets =
                  this.creditsOnClientOnSociete.reduce(
                    (sum, credit) => sum + credit.interet,
                    0,
                  )
                const creditSumSocieteAutresEngagements =
                  this.creditsOnClientOnSociete.reduce(
                    (sum, credit) => sum + credit.autres_engagements,
                    0,
                  )
                const creditSumSocieteExigibles =
                ...
                this.agences.forEach((item) => {
                  const filterCreditsOnClientOnAgenceFinancier: ICreditCustom[] =
                    this.creditsOnClientOnSociete.filter(
                      (point) => point.agenceId === item.id,
                    )

                  this.creditGroupsAgenceFinancier[item.id] =
                    filterCreditsOnClientOnAgenceFinancier

                  const creditSumAgenceCapitals =
                    filterCreditsOnClientOnAgenceFinancier.reduce(
                      (sum, credit) => sum + credit.capital,
                      0,
                    )

                  const creditSumAgenceMoratoires =
                    filterCreditsOnClientOnAgenceFinancier.reduce(
                      (sum, credit) => sum + credit.moratoire,
                      0,
                    )
                  const creditSumAgenceInterets =
                    filterCreditsOnClientOnAgenceFinancier.reduce(
                      (sum, credit) => sum + credit.interet,
                      0,
                    )
                  const creditSumAgenceAutresEngagements =
                    filterCreditsOnClientOnAgenceFinancier.reduce(
                      (sum, credit) => sum + credit.autres_engagements,
                      0,
                    )
                  const creditSumAgenceExigibles =
  ...
  fillerPointByAgence(idAgence: number) {
    const retourPoint: IPointAgence[] = this.pointAgence.filter(
      (point) => point.agenceId === idAgence,
    )
  ...
  showDialogCredit(action: string, creditOnClient: ICreditCustom) {
    ...
    if (action === 'add') {
      ...
      const myVariete: IVariete = {
        ...
        pu_unite: 0,
        isActive: true,
        produitId: creditOnClient.exploitationProduitId,
        ...
        produit: {
          ...
          isEnsachage: false,
          isActive: true,
          filiereId: creditOnClient.exploitationFiliereId,
          filiere: {
...
*/
  ...
  filterOp(event: AutoCompleteCompleteEvent) {
    ...
    const colonnesOps = this.ops.map(({ id, name }) => ({ id, name }))
    // console.log('this.ops: ', this.ops);
    for (let i = 0; i < (colonnesOps as any[]).length; i++) {
      const op = (colonnesOps as any[])[i]
  ...
  supprimerCredit() {
    ...
    body.anneeId = this.loginService.selectedCampagne.anneeId
    body.saisonId = this.loginService.selectedCampagne.saisonId
    body.producteurId = 0
    body.opId = this.creditForm.controls['opCredit']['value']['id']
    console.log('body credit --- ', body)
    this.store.dispatch(
      fromExploitations.deleteExploitation({ exploitation: body }),
    )
  ...
  modifierCredit() {
    ...
    console.log('body credit --- ', body)
    this.store.dispatch(
      fromExploitations.updateExploitation({ exploitation: body }),
    )
