...
export class FamilleEmplacementService {
  ...
  async getOne(familleEmplacementId: number) {
    const familleEmplacement =
      await this.prismaService.familleEmplacement.findUnique({
        where: { id: familleEmplacementId },
      })
  ...
  async update(
    ...
    updateFamilleEmplacementDto: UpdateFamilleEmplacementDto,
  ) {
    const familleEmplacement =
      await this.prismaService.familleEmplacement.findUnique({
        where: { id: familleEmplacementId },
      })
    if (!familleEmplacement)
      throw new NotFoundException('FamilleEmplacement not found')
    await this.prismaService.familleEmplacement.update({
      where: { id: familleEmplacementId },
      data: { ...updateFamilleEmplacementDto },
    })
  ...
  async delete(familleEmplacementId: number) {
    ...
    if (!familleEmplacement) throw new NotFoundException('Post not found')
    await this.prismaService.familleEmplacement.delete({
      where: { id: familleEmplacementId },
    })
