...
import { CreateSousZoneDto, UpdateSousZoneDto } from './sous_zone.dto'
...
export class SousZoneService {
  ...
  async getOne(sousZoneId: number) {
    const sousZone = await this.prismaService.sousZone.findUnique({
      where: { id: sousZoneId },
    })
  ...
  async update(sousZoneId: number, updateSousZoneDto: UpdateSousZoneDto) {
    ...
    await this.prismaService.sousZone.update({
      where: { id: sousZoneId },
      data: { ...updateSousZoneDto },
    })
