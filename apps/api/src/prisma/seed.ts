...
async function deleteExploitation() {
  ...
  console.log(' -----------DELETE FAMILLE TYPE CHARGE EXPLOITATION -----------')
...
async function addConstants() {
  ...
  console.log(' add campagne: 2024 || HIV ')
  const campagne_2024_hiv = await prisma.campagne.create({
    data: {
      anneeId: 24,
      saisonId: 1,
    },
...
async function addLocalite() {
  ...
  console.log(' ---------> add Point Agence: SAINT LOUIS')
  const point_agence_2prd_saint_louis = await prisma.pointAgence.create({
    data: {
      pointId: point_3prd.id,
      agenceId: 1,
    },
  ...
  let m = 0
  while (m < 2) {
    console.log(' ---------> add Op: Op - sv ' + m + ' ' + localite_tilene.name)
    await prisma.op.create({
      data: {
