...
export class PointAgenceService {
  ...
  async getOne(pointAgenceId: number) {
    const pointAgence = await this.prismaService.pointAgence.findUnique({
      where: { id: pointAgenceId },
    })
    if (!pointAgence) throw new NotFoundException('Post not found')
    return pointAgence
  }
  async create(createPointAgenceDto: CreatePointAgenceDto) {
    const { pointId, agenceId } = createPointAgenceDto
  ...
  async update(
    ...
    if (!pointAgence) throw new NotFoundException('PointAgence not found')
