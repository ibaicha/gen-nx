...
import { CreateTypeRemboursementDto, UpdateTypeRemboursementDto } from './type_remboursement.dto';
...
export class TypeRemboursementService {
  ...
  async getOne(typeRemboursementId: number) {
    const typeRemboursement =
      await this.prismaService.typeRemboursement.findUnique({
        where: { id: typeRemboursementId },
      });
  ...
  async update(
    ...
    updateTypeRemboursementDto: UpdateTypeRemboursementDto
  ) {
    const typeRemboursement =
      await this.prismaService.typeRemboursement.findUnique({
        where: { id: typeRemboursementId },
    ...
    if (!typeRemboursement)
      throw new NotFoundException('TypeRemboursement not found');
    await this.prismaService.typeRemboursement.update({
      where: { id: typeRemboursementId },
      data: { ...updateTypeRemboursementDto },
    });
  ...
  async delete(typeRemboursementId: number) {
    ...
    if (!typeRemboursement) throw new NotFoundException('Post not found');
    await this.prismaService.typeRemboursement.delete({
      where: { id: typeRemboursementId },
    });
