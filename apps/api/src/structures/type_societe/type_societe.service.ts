...
import { CreateTypeSocieteDto, UpdateTypeSocieteDto } from './typeSociete.dto';
...
export class TypeSocieteService {
  ...
  async getOne(typeSocieteId: number) {
    const typeSociete = await this.prismaService.typeSociete.findUnique({
      where: { id: typeSocieteId },
    });
  ...
  async update(
    ...
    updateTypeSocieteDto: UpdateTypeSocieteDto
  ) {
    const typeSociete = await this.prismaService.typeSociete.findUnique({
      where: { id: typeSocieteId },
    });
    if (!typeSociete) throw new NotFoundException('TypeSociete not found');
    await this.prismaService.typeSociete.update({
      where: { id: typeSocieteId },
      data: { ...updateTypeSocieteDto },
    });
  ...
  async delete(typeSocieteId: number) {
    ...
    if (!typeSociete) throw new NotFoundException('Post not found');
    await this.prismaService.typeSociete.delete({
      where: { id: typeSocieteId },
    });
