...
import { CreateSocieteDto, UpdateSocieteDto } from './societe.dto';
...
export class SocieteService {
  ...
  async create(createSocieteDto: CreateSocieteDto) {
    const { name, sigle, email, telephone, adresse, typeSocieteId } =
      createSocieteDto;
    await this.prismaService.societe.create({
      data: { name, sigle, email, telephone, adresse, typeSocieteId },
    });
