...
import { CreateTypeOpDto, UpdateTypeOpDto } from './type_op.dto';
...
export class TypeOpService {
  ...
  async getOne(typeOpId: number) {
    const typeOp = await this.prismaService.typeOp.findUnique({
      where: { id: typeOpId },
    });
  ...
  async update(typeOpId: number, updateTypeOpDto: UpdateTypeOpDto) {
    ...
    await this.prismaService.typeOp.update({
      where: { id: typeOpId },
      data: { ...updateTypeOpDto },
    });
